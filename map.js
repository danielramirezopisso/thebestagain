...
function toggleAddMode() {
  ...
  // If turning OFF, clear selection + preview marker
  if (!ADD_MODE) {
    LAST_CLICK = null;
...
async function initMap() {
  ...
  // Click handler for adding (only when Add mode is ON)
  MAP.on("click", (e) => {
    if (!ADD_MODE) return;
    ...
    // Preview marker so you see where you clicked
    if (PREVIEW_MARKER) {
...
async function saveMapMarker() {
  ...
  // Remove preview marker after saving
  if (PREVIEW_MARKER) {
    MAP.removeLayer(PREVIEW_MARKER);
    PREVIEW_MARKER = null;
  }
